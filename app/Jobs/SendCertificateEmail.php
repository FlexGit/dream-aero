...
class SendCertificateEmail extends Job implements ShouldQueue {
	...
	public function handle() {
		...
		$certificateRulesFile->text($period, 320, 250, function ($font) use ($fontPath) {
			$font->file($fontPath);
			$font->size(20);
			$font->color('#000000');
		});
		$certificateRulesFile->text($peopleCount, 750, 330, function ($font) use ($fontPath) {
			$font->file($fontPath);
			$font->size(20);
			$font->color('#000000');
		});
		...
		$certificateRulesFile->text($cityPhone ?? '', 600, 380, function ($font) use ($fontPath) {
			$font->file($fontPath);
			$font->size(20);
			$font->color('#000000');
		});
