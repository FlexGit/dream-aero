...
class CreatePlatformLogsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('platform_logs', function (Blueprint $table) {
			...
			$table->time('start_at')->nullable()->comment('время начала действия');
			$table->time('stop_at')->nullable()->comment('время окончания действия');
			$table->time('duration')->nullable()->comment('длительность');
			$table->timestamps();
			$table->softDeletes();
        });
    }
