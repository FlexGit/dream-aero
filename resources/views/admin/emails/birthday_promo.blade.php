							...
							<tr>
								...
								<th colspan="3" rowspan="3" bgcolor="#FFFFFF" style="padding:10px 0 30px;text-align:center">
									...
									<p style="margin:15px 30px 33px;text-align:left;font-size:16px;line-height:30px;color:#484a42;">
										Здравствуйте{{ $contractor->name ? ', ' . $contractor->name : '' }}!
									</p>
									<p style="margin:15px 30px 33px;text-align:left;font-size:14px;line-height:30px;color:#484a42;">
										Поздравляем с наступающим Днем Рождения! Спешим напомнить, что с сегодняшнего дня и до {{ \Carbon\Carbon::now()->addDays(3)->format('d.m.Y') }} у вас есть возможность забронировать полет со скидкой <span style="font-size: 18px;font-weight: bold;">{{ ($promo->discount->valueFormatted() ?? '') }}</span> в честь праздника.
										<br><br>
