<table bgcolor="#f2f6fc" border="0" cellpadding="5" cellspacing="0" width="100%" style="background:#f2f6fc;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
	<tbody>
	<tr style="border-color:transparent">
		<td align="center" style="border-collapse:collapse;border-color:transparent">
			<table bgcolor="#ffffff" cellpadding="0" cellspacing="0" width="600" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
				<tbody>
				<tr style="border-color:transparent">
					<td border="0" style="border-collapse:collapse;border-color:transparent">
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5;width:100%">
							<tbody>
							<tr style="border-color:transparent">
								<th align="left" valign="top" width="600" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table bgcolor="#ffffff" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="35" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
														<td>&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
														<td valign="top" width="540" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<div align="center" style="color:#333333;font-size:16px;height:75;line-height:1.5;text-align:center;width:100%">
																<div>
																	<a href="https://dream-aero.ru/{{ $event->city ? $event->city->alias : 'msk' }}?utm_source=dreamaero&amp;utm_medium=email&amp;utm_campaign=feedback" style="display:flex;width:167px;height:auto;margin:0 auto;" target="_blank" rel="noopener noreferrer">
																		<img src="https://dream-aero.ru/assets/img/logo-new-mail.png" width="243" alt="logo" align="center" border="0" style="border:0;height:auto;line-height:100%;text-decoration:none;">
																	</a>
																</div>
															</div>
														</td>
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
							</tr>
							</tbody>
						</table>
					</td>
				</tr>
				<tr style="border-color:transparent">
					<td border="0" style="border-collapse:collapse;border-color:transparent">
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5;width:100%">
							<tbody>
							<tr style="border-color:transparent">
								<th align="left" valign="top" width="600" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
														<td valign="top" width="540" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<div style="color:#333333;font-size:16px;line-height:1.5;width:100%">
																<img align="left" alt="123" border="0" src="https://dream-aero.ru/assets/img/leave_review_img1.jpg" width="540" style="border:0;height:auto;line-height:100%;margin:0;text-decoration:none;">
															</div>
														</td>
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
							</tr>
							</tbody>
						</table>
					</td>
				</tr>
				<tr style="border-color:transparent">
					<td border="0" style="border-collapse:collapse;border-color:transparent">
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5;width:100%">
							<tbody>
							<tr style="border-color:transparent">
								<th align="left" valign="top" width="600" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table bgcolor="#ffffff" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;font-weight:normal;line-height:1.5;margin:0">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
														<td valign="top" width="540" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<p style="font-size:inherit;font-weight:normal;line-height:inherit;margin:0 0 10px 0;padding:0"><strong>Добрый день, {{ $event->contractor ? $event->contractor->name . '!' : '' }}</strong></p>
															<p style="font-size:inherit;font-weight:normal;line-height:inherit;margin:0 0 10px 0;padding:0">
																Недавно вы испытывали себя в роли пилота авиалайнера.
																@if($event->score)
																	Мы начислили вам за этот полет [{{ $event->score->score }}] баллов. Воспользоваться ими можно в нашем <a href="https://dream-aero.ru/news/prilozhenie-dreamaero" target="_blank" rel=" noopener noreferrer">мобильном приложении.</a>
																@endif
																<br><br>
																Вам понравилось у нас? Порекомендуете нас друзьям? Поделитесь впечатлениями, нам это очень важно!</p>
														</td>
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
							</tr>
							</tbody>
						</table>
					</td>
				</tr>
				<tr style="border-color:transparent">
					<td border="0" style="border-collapse:collapse;border-color:transparent">
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5;width:100%">
							<tbody>
							<tr style="border-color:transparent">
								<th align="left" valign="top" width="600" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="10" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
														<td valign="top" width="540" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<table align="left" border="0" cellpadding="0" cellspacing="0" height="46" style="background:#fc5c63;border:0 #ddd;border-collapse:collapse;border-radius:4px;color:#333;font-size:16px;line-height:1.5">
																<tbody>
																<tr style="border-color:transparent">
																	<td width="21" style="border:0 transparent;border-collapse:collapse;padding:0;width:21px">&nbsp;&nbsp;</td>
																	<td align="center" height="46" valign="middle" style="border:0 transparent;border-collapse:collapse;border-radius:4px;height:46px;padding:0;text-align:center;vertical-align:middle;width:auto">
																		<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:0;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
																			<tbody>
																			<tr style="border-color:transparent">
																				<td align="center" style="border:0 transparent;border-collapse:collapse;line-height:1;padding:0"><a href="https://yandex.ru/maps/-/CCUQ7FeggA" target="_blank" style="color:#ffffff;font-size:18px;font-weight:normal;text-decoration:none" rel=" noopener noreferrer">Оставить отзыв</a></td>
																			</tr>
																			</tbody>
																		</table>
																	</td>
																	<td width="21" style="border:0 transparent;border-collapse:collapse;padding:0;width:21px">&nbsp;&nbsp;</td>
																</tr>
																</tbody>
															</table>
														</td>
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<div style="color:#333;font-size:16px;line-height:1.5;padding:15px">
													<hr style="border-style:solid none none none;border-top-color:#b2b2b2;border-width:1px 0 0 0;margin:0">
												</div>
											</td>
										</tr>
										</tbody>
									</table>
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="10" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
							</tr>
							</tbody>
						</table>
					</td>
				</tr>
				<tr style="border-color:transparent">
					<td border="0" style="border-collapse:collapse;border-color:transparent">
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5;width:100%">
							<tbody>
							<tr style="border-color:transparent">
								<th align="left" valign="top" width="600" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;font-weight:normal;line-height:1.5;margin:0">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="15" style="border-collapse:collapse;border-color:transparent;width:15px !important">&nbsp;</td>
														<td valign="top" width="570" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<h2 style="color:#333;font-size:30px;font-weight:normal;line-height:1.2;margin:0 0 10px 0">Также предлагаем вам узнать подробнее о наших курсах:</h2>
														</td>
														<td height="100%" width="15" style="border-collapse:collapse;border-color:transparent;width:15px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
							</tr>
							</tbody>
						</table>
					</td>
				</tr>
				<tr style="border-color:transparent">
					<td border="0" style="border-collapse:collapse;border-color:transparent">
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5;width:100%">
							<tbody>
							<tr style="border-color:transparent">
								<th align="left" valign="top" width="200" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
														<td valign="top" width="160" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<div style="color:#333333;font-size:16px;height:155;line-height:1.5;width:100%">
																<img align="left" alt="letaemkurs_1" border="0" src="https://dream-aero.ru/assets/img/leave_review_img2.jpg" width="160" style="border:0;height:auto;line-height:100%;margin:0;text-decoration:none">
															</div>
														</td>
														<td height="100%" width="10" style="border-collapse:collapse;border-color:transparent;width:10px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
								<th align="left" valign="top" width="400" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;font-weight:normal;line-height:1.5;margin:0">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="10" style="border-collapse:collapse;border-color:transparent;width:10px !important">&nbsp;</td>
														<td valign="top" width="360" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<h3 style="color:#333;font-size:24px;font-weight:normal;line-height:1.5;margin:0 0 10px 0"><strong>Видео курс "Летаем без страха"</strong></h3>
															<p style="font-size:inherit;font-weight:normal;line-height:1.5;margin:0 0 10px 0;padding:0">Самый подробный видеокурс для лечения аэрофобии от лучшего специалиста в России!</p>
														</td>
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="15" style="border-collapse:collapse;border-color:transparent;width:15px !important">&nbsp;</td>
														<td valign="top" width="370" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<div>
																<table border="0" cellpadding="0" cellspacing="0" height="28" style="background:#888;border:0;border-collapse:collapse;border-radius:5px;color:#333;font-size:16px;line-height:1.5;margin-left:auto;margin-right:auto">
																	<tbody>
																	<tr style="border-color:transparent">
																		<td width="21" style="border:0 transparent;border-collapse:collapse;padding:0;width:21px">&nbsp;&nbsp;</td>
																		<td align="center" height="28" valign="middle" style="border:0 transparent;border-collapse:collapse;border-radius:5px;height:28px;padding:0;text-align:center;vertical-align:middle;width:auto">
																			<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:0;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
																				<tbody>
																				<tr style="border-color:transparent">
																					<td align="center" style="border:0 transparent;border-collapse:collapse;line-height:1;padding:0"><a href="https://dream-aero.ru/lechenie-aerofobii?utm_source=dreamaero&utm_medium=email&utm_campaign=feedback" target="_blank" style="color:#ffffff;font:bold 16px 'arial' , 'helvetica neue' , 'helvetica' , sans-serif;text-decoration:none" rel="noopener noreferrer">Подробнее</a></td>
																				</tr>
																				</tbody>
																			</table>
																		</td>
																		<td width="21" style="border:0 transparent;border-collapse:collapse;padding:0;width:21px">&nbsp;&nbsp;</td>
																	</tr>
																	</tbody>
																</table>
															</div>
														</td>
														<td height="100%" width="15" style="border-collapse:collapse;border-color:transparent;width:15px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
							</tr>
							</tbody>
						</table>
					</td>
				</tr>
				<tr style="border-color:transparent">
					<td border="0" style="border-collapse:collapse;border-color:transparent">
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5;width:100%">
							<tbody>
							<tr style="border-color:transparent">
								<th align="left" valign="top" width="200" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
														<td valign="top" width="160" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<div style="color:#333333;font-size:16px;height:199;line-height:1.5;width:100%">
																<img align="left" alt="1_013f751a79c87e6b14" border="0" src="https://dream-aero.ru/assets/img/leave_review_img3.jpg" width="160" style="border:0;height:auto;line-height:100%;margin:0;text-decoration:none">
															</div>
														</td>
														<td height="100%" width="10" style="border-collapse:collapse;border-color:transparent;width:10px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
								<th align="left" valign="top" width="400" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;font-weight:normal;line-height:1.5;margin:0">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="10" style="border-collapse:collapse;border-color:transparent;width:10px !important">&nbsp;</td>
														<td valign="top" width="360" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<h3 style="color:#333;font-size:24px;font-weight:normal;line-height:1.5;margin:0 0 10px 0"><strong>Курс пилота "Basic"</strong></h3>
															<p style="font-size:inherit;font-weight:normal;line-height:1.5;margin:0 0 10px 0;padding:0">Шесть часов:</p>
															<ul>
																<li style="font-weight:normal;line-height:1.5">Основы аэродинамики</li>
																<li style="font-weight:normal;line-height:1.5">Общие принципы устройства самолётов Боинг</li>
																<li style="font-weight:normal;line-height:1.5">Основные правила выполнения полётов</li>
																<li style="font-weight:normal;line-height:1.5">Научитесь выполнять основные элементы полётных заданий</li>
																<li style="font-weight:normal;line-height:1.5">Освоите визуальную и приборную визуализацию при управлении самолётом</li>
																<li style="font-weight:normal;line-height:1.5">Научитесь взлетать и производить посадку</li>
															</ul>
														</td>
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="15" style="border-collapse:collapse;border-color:transparent;width:15px !important">&nbsp;</td>
														<td valign="top" width="370" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<div>
																<table border="0" cellpadding="0" cellspacing="0" height="28" style="background:#888;border:0;border-collapse:collapse;border-radius:5px;color:#333;font-size:16px;line-height:1.5;margin-left:auto;margin-right:auto">
																	<tbody>
																	<tr style="border-color:transparent">
																		<td width="21" style="border:0 transparent;border-collapse:collapse;padding:0;width:21px">&nbsp;&nbsp;</td>
																		<td align="center" height="28" valign="middle" style="border:0 transparent;border-collapse:collapse;border-radius:5px;height:28px;padding:0;text-align:center;vertical-align:middle;width:auto">
																			<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:0;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
																				<tbody>
																				<tr style="border-color:transparent">
																					<td align="center" style="border:0 transparent;border-collapse:collapse;line-height:1;padding:0"><a href="https://dream-aero.ru/{{ $event->city ? $event->city->alias : 'msk' }}/price?utm_source=dreamaero&utm_medium=email&utm_campaign=feedback" target="_blank" style="color:#ffffff;font:bold 16px 'arial' , 'helvetica neue' , 'helvetica' , sans-serif;text-decoration:none" rel="noopener noreferrer">Подробнее</a></td>
																				</tr>
																				</tbody>
																			</table>
																		</td>
																		<td width="21" style="border:0 transparent;border-collapse:collapse;padding:0;width:21px">&nbsp;&nbsp;</td>
																	</tr>
																	</tbody>
																</table>
															</div>
														</td>
														<td height="100%" width="15" style="border-collapse:collapse;border-color:transparent;width:15px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
							</tr>
							</tbody>
						</table>
					</td>
				</tr>
				<tr style="border-color:transparent">
					<td border="0" style="border-collapse:collapse;border-color:transparent">
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5;width:100%">
							<tbody>
							<tr style="border-color:transparent">
								<th align="left" valign="top" width="200" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
														<td valign="top" width="160" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<div style="color:#333333;font-size:16px;height:239;line-height:1.5;width:100%">
																<img align="left" alt="321" border="0" src="https://dream-aero.ru/assets/img/leave_review_img4.jpg" width="160" style="border:0;height:auto;line-height:100%;margin:0;text-decoration:none">
															</div>
														</td>
														<td height="100%" width="10" style="border-collapse:collapse;border-color:transparent;width:10px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
								<th align="left" valign="top" width="400" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;font-weight:normal;line-height:1.5;margin:0">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="10" style="border-collapse:collapse;border-color:transparent;width:10px !important">&nbsp;</td>
														<td valign="top" width="360" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<h3 style="color:#333;font-size:24px;font-weight:normal;line-height:1.5;margin:0 0 10px 0"><strong>Курс пилота "Expert"</strong></h3>
															<p style="font-size:inherit;font-weight:normal;line-height:1.5;margin:0 0 10px 0;padding:0">Девять часов:</p>
															<ul>
																<li style="font-weight:normal;line-height:1.5">научитесь производить посадку в простых и сложных метеоусловиях (как при низкой видимости, так и при сильном ветре)</li>
																<li style="font-weight:normal;line-height:1.5">основным процедурам взаимодействия в экипаже согласно SOP (Standard Operating Procedures) авиакомпании</li>
																<li style="font-weight:normal;line-height:1.5">использовать схемы Jeppesen в части касающейся полетов в зоне аэродрома</li>
																<li style="font-weight:normal;line-height:1.5">использовать FMC для полета по маршруту</li>
															</ul>
														</td>
														<td height="100%" width="30" style="border-collapse:collapse;border-color:transparent;width:30px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="20" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
													<tbody>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td height="100%" width="15" style="border-collapse:collapse;border-color:transparent;width:15px !important">&nbsp;</td>
														<td valign="top" width="370" style="border-collapse:collapse;border-color:transparent;vertical-align:top">
															<div>
																<table border="0" cellpadding="0" cellspacing="0" height="28" style="background:#888;border:0;border-collapse:collapse;border-radius:5px;color:#333;font-size:16px;line-height:1.5;margin-left:auto;margin-right:auto">
																	<tbody>
																	<tr style="border-color:transparent">
																		<td width="21" style="border:0 transparent;border-collapse:collapse;padding:0;width:21px">&nbsp;&nbsp;</td>
																		<td align="center" height="28" valign="middle" style="border:0 transparent;border-collapse:collapse;border-radius:5px;height:28px;padding:0;text-align:center;vertical-align:middle;width:auto">
																			<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:0;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
																				<tbody>
																				<tr style="border-color:transparent">
																					<td align="center" style="border:0 transparent;border-collapse:collapse;line-height:1;padding:0"><a href="https://dream-aero.ru/{{ $event->city ? $event->city->alias : 'msk' }}/price?utm_source=dreamaero&utm_medium=email&utm_campaign=feedback" style="color:#ffffff;font:bold 16px 'arial' , 'helvetica neue' , 'helvetica' , sans-serif;text-decoration:none" target="_blank" rel="noopener noreferrer">Подробнее</a></td>
																				</tr>
																				</tbody>
																			</table>
																		</td>
																		<td width="21" style="border:0 transparent;border-collapse:collapse;padding:0;width:21px">&nbsp;&nbsp;</td>
																	</tr>
																	</tbody>
																</table>
															</div>
														</td>
														<td height="100%" width="15" style="border-collapse:collapse;border-color:transparent;width:15px !important">&nbsp;</td>
													</tr>
													<tr style="border-color:transparent">
														<td colspan="3" height="15" width="100%" style="border-collapse:collapse;border-color:transparent">&nbsp;</td>
													</tr>
													</tbody>
												</table>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
							</tr>
							</tbody>
						</table>
					</td>
				</tr>
				<tr style="border-color:transparent">
					<td border="0" style="border-collapse:collapse;border-color:transparent">
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;color:#333;font-size:16px;line-height:1.5;width:100%">
							<tbody>
							<tr style="border-color:transparent">
								<th align="left" valign="top" width="600" style="border-color:transparent;font-weight:400;text-align:left;vertical-align:top">
									<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-collapse:collapse;color:#333;font-size:16px;line-height:1.5">
										<tbody>
										<tr style="border-color:transparent">
											<td style="border-collapse:collapse;border-color:transparent">
												<div style="color:#333;font-size:16px;line-height:1.5;padding:15px 30px 15px 30px">
													<hr style="border-style:dashed none none none;border-top-color:#b2b2b2;border-width:1px 0 0 0;margin:0">
													<span style="box-sizing:border-box;font-size:13px">Вы получили данную рассылку, так как являетесь клиентом или подписчиком Dream Aero.</span>
													<p><a href="https://dream-aero.ru/unsubscribe?contractor_id={{ $event->contractor_id }}" target="_blank" rel="noopener noreferrer">Отписаться от рассылки</a></p>
												</div>
											</td>
										</tr>
										</tbody>
									</table>
								</th>
							</tr>
							</tbody>
						</table>
					</td>
				</tr>
				</tbody>
			</table>
		</td>
	</tr>
	</tbody>
</table>