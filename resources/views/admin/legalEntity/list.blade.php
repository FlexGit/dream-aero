...
<tr class="odd">
	...
	<td class="text-center">{{ $legalEntity->alias }}</td>
	<td class="text-center">{{ $legalEntity->is_active ? 'Да' : 'Нет' }}</td>
	<td class="text-center">{!! ($legalEntity->data_json && array_key_exists('public_offer_file_path', $legalEntity->data_json)) ? '<a href="' . \URL::to('/upload/' . $legalEntity->data_json['public_offer_file_path']) . '" target="_blank">ссылка</a>' : '' !!}</td>
