...
<input type="hidden" id="is_unified" name="is_unified" value="{{ ($position->certificate && $position->certificate->city_id) ? 0 : 1 }}">
		...
		<div class="form-group">
			...
			<select class="form-control" id="city_id" name="city_id">
				...
				@foreach($cities ?? [] as $city)
					<option value="{{ $city->id }}" @if($position->certificate && $city->id == $position->certificate->city_id) selected @endif>{{ $city->name }}</option>
				@endforeach
			</select>
		</div>
	</div>
