	...
	<fieldset>
			...
			<div style="width: 100%;">
				<select id="product" name="product" class="popup-input">
					@foreach($products as $product)
						<option value="{{ $product->duration }}" data-product-id="{{ $product->id }}">{{ $product->duration }} мин</option>
					@endforeach
				</select>
			</div>
		</div>
		...
		<div class="consent-container">
			<label class="cont">
				...
				@endforeach--}}
				<input type="checkbox" id="" name="consent" value="1" {{--{{ ($checkedLocation && $checkedSimulator) ? 'checked' : '' }}--}}>
				<span class="checkmark"></span>
			</label>
		</div>
		<button type="button" onclick="yaCounter46672077.reachGoal('SendOrder'); gtag_report_conversion();fbq('track', 'Purchase', {value: 200,currency: 'rub',});return true;" class="popup-submit button-pipaluk button-pipaluk-grey"><i>Отправить</i></button>
